<?php
require('fpdf.php');

class PDF extends FPDF
{
// Cabecera de página
function Header()
{
	
    // Arial bold 15
    $this->SetFont('Arial','B',10);
    // Movernos a la derecha
    $this->Cell(60);
    //Logo
    $this->Image('Logo-VidadePaz.jpeg', 185, 5, 20);
    // Título
    $this->Cell(70,10,'Reporte de Familiares ',0,0,'C');
    // Salto de línea
    $this->Ln(20);

    $this->Cell(20,10,'Nombre',1,0,'C',0);
	$this->Cell(23,10,'ApePaterno',1,0,'C',0);
	$this->Cell(23,10,'ApeMaterno',1,0,'C',0);
    $this->Cell(20,10,'DNI',1,0,'C',0);
    $this->Cell(60,10,'Correo',1,0,'C',0);
    $this->Cell(20,10,'Telefono',1,0,'C',0);
    $this->Cell(30,10,'DNI Paciente',1,1,'C',0);
}

// Pie de página
function Footer()
{
    // Posición: a 1,5 cm del final
    $this->SetY(-15);
    // Arial italic 8
    $this->SetFont('Arial','I',8);
    // Número de página
    $this->Cell(0,10,utf8_decode('Página') .$this->PageNo().'/{nb}',0,0,'C');
}
}

require ("cn.php");
$consulta = "SELECT * FROM familiares";
$resultado = mysqli_query($conexion, $consulta);

$pdf = new PDF();
$pdf->AliasNbPages();
$pdf->AddPage();
$pdf->SetFont('Arial','B',10);

while ($row=$resultado->fetch_assoc()) {
	$pdf->Cell(20,10,$row['nombre'],1,0,'C',0);
	$pdf->Cell(23,10,$row['apellidop'],1,0,'C',0);
	$pdf->Cell(23,10,$row['apellidom'],1,0,'C',0);
    $pdf->Cell(20,10,$row['dni'],1,0,'C',0);
    $pdf->Cell(60,10,$row['correo'],1,0,'C',0);
    $pdf->Cell(20,10,$row['telefono'],1,0,'C',0);
    $pdf->Cell(30,10,$row['dnipaciente'],1,1,'C',0);
  

} 


	$pdf->Output();
?>